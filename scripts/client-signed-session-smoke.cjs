...
(async () => {
  ...
  // ===== Phase 3 =====
  // Legacy /api/identity/signin was removed in migration step 5.
  // The route should fall through to the catch-all 404. /api/identity/signup
  // stays mounted (covered by client-signup-no-server-key Phase 2a),
  // but POSTing to /signin should now return a 404, NOT a 200 or
  // 401 with sessionToken. This phase is the death-watch canary.
  // /dev/signin alias is also gone.
  const removedSigninResp = await postJson("/api/identity/signin", { handle: "anything", password: "anything" });
  if (removedSigninResp.status === 404) {
    ok(`legacy POST /api/identity/signin removed (404 from catch-all)`);
  } else {
    fail("legacy-signin-removed", `expected 404, got ${removedSigninResp.status}: ${JSON.stringify(removedSigninResp.body)}`);
  }
  const removedAliasResp = await postJson("/dev/signin", { handle: "anything", password: "anything" });
  if (removedAliasResp.status === 404) {
    ok(`/dev/signin alias also removed (404)`);
  } else {
    fail("legacy-alias-removed", `expected 404, got ${removedAliasResp.status}`);
  }

  // ===== Phase 4: legacy signup + recover removed =====
  // Migration step 6 deleted POST /api/identity/signup and
  // POST /api/identity/recover (and their /dev/* aliases). Telemetry
  // over the 5dde8a3 release window showed only sudo-probe-* user
  // agents — zero Mozilla/* across both routes — so the password and
  // backup-code-recovery surfaces were retired. The death-watch
  // canary now confirms all four POSTs fall through to the catch-all
  // 404. The [legacy-signup] and [legacy-recover] structured logs
  // are gone with the handlers, so there's nothing left to assert
  // about the local server log.
  const removedSignupResp = await postJson("/api/identity/signup", {
    handle: "anything", password: "Anything1!", recoveryQuestion: "q", recoveryAnswer: "a"
  });
  if (removedSignupResp.status === 404) {
    ok(`legacy POST /api/identity/signup removed (404 from catch-all)`);
  } else {
    fail("legacy-signup-removed", `expected 404, got ${removedSignupResp.status}: ${JSON.stringify(removedSignupResp.body)}`);
  }
  const removedSignupAliasResp = await postJson("/dev/signup", {
    handle: "anything", password: "Anything1!", recoveryQuestion: "q", recoveryAnswer: "a"
  });
  if (removedSignupAliasResp.status === 404) {
    ok(`/dev/signup alias also removed (404)`);
  } else {
    fail("legacy-signup-alias-removed", `expected 404, got ${removedSignupAliasResp.status}`);
  }
  const removedRecoverResp = await postJson("/api/identity/recover", {
    handle: "anything", backupCode: "x", recoveryQuestion: "q", recoveryAnswer: "a"
  });
  if (removedRecoverResp.status === 404) {
    ok(`legacy POST /api/identity/recover removed (404 from catch-all)`);
  } else {
    fail("legacy-recover-removed", `expected 404, got ${removedRecoverResp.status}: ${JSON.stringify(removedRecoverResp.body)}`);
  }
  const removedRecoverAliasResp = await postJson("/dev/recover", {
    handle: "anything", backupCode: "x", recoveryQuestion: "q", recoveryAnswer: "a"
  });
  if (removedRecoverAliasResp.status === 404) {
    ok(`/dev/recover alias also removed (404)`);
  } else {
    fail("legacy-recover-alias-removed", `expected 404, got ${removedRecoverAliasResp.status}`);
  }
