...
const LOCAL_CHAT_TARGETS_STORAGE_KEY = "sudo.local.chatTargets";
...
async function runLookup(rawQuery: string): Promise<void> {
  ...
  try {
    ...
    const fingerprint = await fingerprintPublicKey(identity.public_key);
...
async function runSignup(
  ...
  try {
    ...
    // DEV ONLY: localStorage is used for iteration so reloads can restore the
    // dev account. Production should bind sessions to device-held credentials.
    localStorage.setItem(DEV_SESSION_TOKEN_STORAGE_KEY, result.sessionToken);
    signupDialog.close();
...
async function runSignin(rawHandle: string, password: string): Promise<void> {
  ...
  try {
    ...
    // dev account. Production should bind sessions to device-held credentials.
    localStorage.setItem(DEV_SESSION_TOKEN_STORAGE_KEY, result.sessionToken);
    const fingerprint = await fingerprintPublicKey(result.identity.public_key);
...
async function restoreStoredSession(): Promise<void> {
  ...
  const token = localStorage.getItem(DEV_SESSION_TOKEN_STORAGE_KEY);
  if (token === null) {
  ...
  try {
    ...
    if (sequence !== authSequence) return;
    localStorage.removeItem(DEV_SESSION_TOKEN_STORAGE_KEY);
    setSignedOut();
  }
}
...
function readLocalChats(): ChatSummary[] {
  const raw = localStorage.getItem(LOCAL_CHAT_TARGETS_STORAGE_KEY);
  if (raw === null) return [];

  try {
    const parsed = JSON.parse(raw) as ChatSummary[];
    if (!Array.isArray(parsed)) return [];
    return parsed.filter((chat) => typeof chat.handle === "string" && typeof chat.id === "string");
  } catch {
    return [];
  }
}

function persistLocalChats(): void {
  localStorage.setItem(LOCAL_CHAT_TARGETS_STORAGE_KEY, JSON.stringify(localChats));
}
